<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="valueList" @selection-change="handleSelectionChange">
      ...
      <!-- <el-table-column label="狀態(0:正常，1：不正常)" align="center" prop="status" /> -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="130px">
        <el-form-item label="原始屬性值數值" prop="originalValueName">
...
<script>
import { listValue, getValue, delValue, addValue, updateValue } from "@/api/product/attributeValue";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        originalValueName: [
          { required: true, message: "原始屬性值數值不能為空", trigger: "blur" }
        ],
        displayValueName: [
          { required: true, message: "顯示屬性值名稱不能為空", trigger: "blur" }
        ],
        valueUnit: [
          { required: true, message: "屬性值單位不能為空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    /** 匯出按鈕操作 */
    handleExport() {
      this.download('product/attributeValue/export', {
        ...this.queryParams
